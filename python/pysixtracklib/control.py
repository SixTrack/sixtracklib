...
class NodeId(object):
    ...
    def to_string( node_id ):
        ...
        if _status == st_ARCH_STATUS_SUCCESS:
            ...
            node_id_str = node_id_str.decode( 'utf-8' )
        return node_id_str
...
class NodeInfoBase(object):
    ...
    def to_string( node_info ):
        ...
        if _ptr_node_info != st_NullNodeInfoBase:
            ...
            if _status == st_ARCH_STATUS_SUCCESS:
                str_repr = str( _temp_str.value )
                str_repr.decode('utf-8')
        return str_repr
    ...
    def arch_str(self):
        ...
        if st_NodeInfo_has_arch_string( self._ptr_node_info ):
            arch_str = str( st_NodeInfo_get_arch_string( self._ptr_node_info ) )
    ...
    def platform_name(self):
        temp_cstr = _st_NodeInfo_get_platform_name( self._ptr_node_info )
        if temp_cstr != st_NullChar:
            platform_name = str( temp_cstr ).decode('utf-8')
        else:
            platform_name = u""
    ...
    def device_name(self):
        temp_cstr = _st_NodeInfo_get_device_name( self._ptr_node_info )
        if temp_cstr != st_NullChar:
            device_name = str( temp_cstr ).decode('utf-8')
        else:
            device_name = u""
        return device_name 
    ...
    def description(self):
        temp_cstr = _st_NodeInfo_get_description( self._ptr_node_info )
        if temp_cstr != st_NullChar:
            description = str( temp_cstr ).decode('utf-8')
        else:
            description = u""
        return description
...
class ControllerBase(object):
    ...
    def arch_str(self):
        ...
        if st_Controller_has_arch_string( self._ptr_ctrl ):
            arch_str = str( st_Controller_get_arch_string( self._ptr_ctrl ) )
            arch_str = arch_str.decode( 'utf-8' )
        return arch_str
...
class NodeControllerBase(ControllerBase):
    ...
    def available_node_indices( ctrl=None ):
        ...
        if _ptr_ctrl != st_NullControllerBase:
            ...
            if av_num_nodes > 0:
                ...
                _status = st_Controller_get_available_node_indices( _ptr_ctrl, 
                   st_arch_size_t( av_num_nodes ), _temp_node_indices )
                if _status == st_ARCH_STATUS_SUCCESS:
    ...
    def is_default_node_check( self, node_id=st_NullNodeId, node_id_str=None, 
    ...
    def is_selected_node_check( self, node_id_str=None, node_id=st_NullNodeId, 
        node_index=None, platform_id=None, device_id=None ):
        ...
        if st_Controller_has_selected_node( self._ptr_ctrl ) and \
           _selected_node_index != st_NODE_UNDEFINED_INDEX.value:
            if node_index is not None and \
                ...
                _is_selected_node = bool( _selected_node_index == st_Controller_get_node_index_by_node_id( 
                    self._ptr_ctrl, node_id.pointer ) )
            elif node_id is not None and node_id != st_NullNodeId:
                _is_selected_node = bool( _selected_node_index ==       
                    st_Controller_get_node_index_by_node_id( 
                        self._ptr_ctrl, node_id ) )                    
            elif node_id_str is not None:
                node_id_str = node_id_str.encode( 'utf-8' )
                _is_selected_node = bool( _selected_node_index ==       
                    st_Controller_get_node_index( self._ptr_ctrl, 
                        ct.c_char_p( node_id_str ) ) )
            elif platform_id is not None and \
                platform_id != st_NODE_ILLEGAL_PLATFORM_ID.value and \
                device_id is not None and \
                device_id != st_NODE_ILLEGAL_DEVICE_ID.value:
    ...
    def is_node_available_check( self, node_id_str=None, node_id=st_NullNodeId, 
        node_index=None, platform_id=None, device_id=None ):
        _is_available=False
        if node_index is not None and \
            node_index != st_NODE_UNDEFINED_INDEX.value:
