...
def dispatch(el, classes):
    ...
    if key == "multipole":
        ...
        el = Multipole(
            knl=knl, ksl=ksl, hxl=knl[0], hyl=0, length=el.lrad
        )
    elif key in [
        "marker",
        "hmonitor",
        ...
        el = Multipole(
            knl=[-el.kick], ksl=[], length=el.lrad, hxl=0, hyl=0
        )
    elif key == "vkicker":
        el = Multipole(
            knl=[], ksl=[el.kick], length=el.lrad, hxl=0, hyl=0
        )
    elif key == "rfcavity":
        el = Cavity(
            voltage=el.volt * 1e6,
            frequency=el.freq * 1e6,
            lag=el.lag * 360,
        )
    return el
