...
class Buffer(object):
    def __init__(
        ...
        if self._cbuffer is not None:
            ...
            ptr_ext_buffer is not None
            and ptr_ext_buffer != st_NullBuffer
        ):
            ...
            self._ptr_st_buffer = st_Buffer_new_from_file(
                _path_to_file
            )
        elif (
            ...
            self._ptr_st_buffer = st_Buffer_new(
                st_buffer_size_t(size)
            )
        elif (
    ...
    @classmethod
    def from_cbuffer(
        cls, obj, owns_buffer=False, owns_pointer=True, **kwargs
    ):
    ...
    def num_garbage_ranges(self):
        return st_Buffer_get_num_of_garbage_ranges(
            self._ptr_st_buffer
        )

    @property
    def max_num_garbage_ranges(self):
        return st_Buffer_get_max_num_of_garbage_ranges(
            self._ptr_st_buffer
        )
    ...
    def ptr_data_begin(self):
        return ct.cast(
            st_Buffer_get_data_begin_addr(self._ptr_st_buffer),
            st_uchar_p,
        )
    ...
    def ptr_data_end(self):
        return ct.cast(
            st_Buffer_get_data_end_addr(self._ptr_st_buffer),
            st_uchar_p,
        )
    ...
    def clear(self, clear_data=False):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "unsuccessful clear op; status:{0}".format(
                    self._last_status
                )
            )
        return self
    ...
    def refresh(self):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "unsuccessful refresh op; status:{0}".format(
                    self._last_status
                )
            )
        return self
    ...
    def remap(self):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "unsuccessful reamp op; status:{0}".format(
                    self._last_status
                )
            )
        return self
    ...
    def write_to_file(self, path_to_file, normalized_base_addr=None):
        if path_to_file is not None and path_to_file != "":
            _path_to_file = ct.c_char_p(path_to_file.encode("utf-8"))
            if (
                normalized_base_addr is not None
                and normalized_base_addr > 0
            ):
    ...
    def reserve(
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "unsuccessful reserve op; status:{0}".format(
                    self._last_status
                )
            )
    ...
    def reset(
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "unsuccessful reset op; status:{0}".format(
                    self._last_status
                )
            )
        return self
