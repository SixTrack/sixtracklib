...
if SIXTRACKLIB_MODULES.get("cuda", False):
    ...
    class CudaNodeInfo(NodeInfoBase):
            ...
            if _ptr_node_info == st_NullCudaNodeInfo:
                if cuda_dev_index is not None and cuda_dev_index > 0:
                    if platform_id is None:
                        platform_id = (
                            st_NODE_ILLEGAL_PLATFORM_ID.value
                        )
                    ...
                    owns_ptr = bool(
                        _ptr_node_info != st_NullCudaNodeInfo
                    )

            if _ptr_node_info != st_NullCudaNodeInfo:
                super().__init__(
                    ptr_node_info=_ptr_node_info,
                    owns_ptr=owns_ptr,
                    **kwargs
                )
        ...
        def cuda_device_index(self):
            return st_CudaNodeInfo_get_cuda_device_index(
                self._ptr_node_info
            )
    ...
    class CudaController(NodeControllerBase):
        ...
        def GET_AVAILABLE_NODES(
            ...
            if _num_avail_nodes > 0:
                ...
                for ii in range(0, _num_avail_nodes):
                    _node_ids[ii] = ct.cast(
                        st_NodeId_create, st_NodeId_p
                    )
        ...
        def __init__(
            ...
            **kwargs
            ...
            if (
                ...
                if node_id is not None and isinstance(
                    node_id, NodeId
                ):
                    ...
                    cuda_dev_index is not None and cuda_dev_index >= 0
                ):
        ...
        def managed_cobject_buffer_needs_remapping(
            ...
            return not st_CudaController_is_managed_cobject_buffer_remapped(
                self._ptr_ctrl,
                ptr_argument,
                st_buffer_size_t(slot_size),
            )

        def remap_managed_cobject_buffer(
            self, ptr_argument, slot_size
        ):
        ...
        def receive_memory(
            self, ptr_mem_begin, ptr_argument, mem_capacity
        ):
            ...
            **kwargs
            ...
            if (
                ...
                if buffer is not None and isinstance(buffer, Buffer):
                    ...
                    ptr_buffer = buffer
                if (
                    ptr_buffer is not None
                    and ptr_buffer != st_NullBuffer
                ):
                ...
                if (
                    _ptr_arg is not None
                    and _ptr_arg != st_NullCudaArgument
                ):
                    kwargs["ptr_argument"] = _ptr_arg
                    kwargs["owns_ptr"] = True
                else:
                    raise ValueError(
                        "Error during creation of CudaArgument"
                    )
            super().__init__(**kwargs)
    ...
    class CudaTrackJob(TrackJobBaseNew):
        def __init__(
            ...
            if _ptr_track_job == st_NullCudaTrackJob:
                raise RuntimeError(
                    "Error while creating CudaTrackJob"
                )
            ...
            if (
                ...
                raise RuntimeError(
                    "CudaTrackJob requires CudaController"
                )

            if node_id_str is not None and node_id_str != "":
                _node_id_str = ct.c_char_p(
                    node_id_str.encode("utf-8")
                )
            ...
            if _last_status == st_ARCH_STATUS_SUCCESS.value:
                super().__init__(
                    ptr_track_job=_ptr_track_job, owns_ptr=True
                )
            ...
            if (
                ...
                raise RuntimeError(
                    "Error during creation of TrackJobBase"
                )
            ...
            if self._last_status != st_ARCH_STATUS_SUCCESS.value:
                raise RuntimeError(
                    "Error during resetting TrackJobBase"
                )
        ...
        def has_particles_arg(self):
            return st_CudaTrackJob_has_particles_arg(
                self._ptr_track_job
            )
        ...
        def particles_arg(self):
            ...
            return CudaArgument(
                ptr_argument=_ptr_argument, owns_ptr=False
            )

        @property
        def has_beam_elements_arg(self):
            return st_CudaTrackJob_has_beam_elements_arg(
                self._ptr_track_job
            )
        ...
        def has_particles_addr_arg(self):
            return st_CudaTrackJob_has_particles_addr_arg(
                self._ptr_track_job
            )
        ...
        def has_debug_register_arg(self):
            return st_CudaTrackJob_has_debug_register_arg(
                self._ptr_track_job
            )
    ...
    class CudaController(object):
        ...
        def NUM_ALL_NODES():
            raise RuntimeError(
                "Cuda module disabled, no nodes present"
            )
            return 0
        ...
        def PRINT_ALL_NODES():
            raise RuntimeError(
                "Cuda module disabled, no nodes to print"
            )
            st_Cuda_print_all_nodes()
        ...
        def NUM_AVAILABLE_NODES(filter_str=None, env_var_name=None):
            raise RuntimeError(
                "Cuda module disabled, no nodes available"
            )
            return 0
