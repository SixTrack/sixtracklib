...
if importlib.util.find_spec("pysixtrack") is not None:
    ...
    class BeamElementConverter(object):
        ...
        def get_elemtype(type_id):
            return BeamElementConverter._type_id_to_elem_map.get(
                type_id, None
            )
        ...
        def to_pysixtrack(elem):
            ...
            if cls is not None:
                if type_id == 2:
                    pysixtrack_elem = pysixelem.Drift(
                        length=elem.length
                    )
                elif type_id == 3:
                    pysixtrack_elem = pysixelem.DriftExact(
                        length=elem.length
                    )
                elif type_id == 4:
                    ...
                    pysixtrack_elem = pysixelem.XYShift(
                        dx=elem.dx, dy=elem.dy
                    )
                elif type_id == 7:
                    pysixtrack_elem = pysixelem.SRotation(
                        angle=elem.angle_deg
                    )
                elif type_id == 8:
        ...
        def from_pysixtrack(elem, cbuffer=None):
            ...
            if (
                elem_type
                in BeamElementConverter._pysixtrack_to_type_id_map
            ):
                ...
                if (
                    type_id
                    in BeamElementConverter._type_id_to_elem_map
                ):
                    cls = BeamElementConverter._type_id_to_elem_map[
                        type_id
                    ]
