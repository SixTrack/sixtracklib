...
class TrackJobBaseNew(object):
    ...
    def _reset_detailed(
        ...
        if (
            ...
            raise ValueError(
                "TrackJob has to be initialized before "
                + "calling reset"
            )
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if particles_buffer is not None:
            if isinstance(particles_buffer, CBuffer):
                ...
                _internal_particles_buffer = Buffer(
                    cbuffer=particles_buffer
                )
                _ptr_c_particles_buffer = (
                    _internal_particles_buffer.pointer
                )
            elif isinstance(particles_buffer, CObject):
                _particles_buffer = particles_buffer._buffer
                _internal_particles_buffer = Buffer(
                    cbuffer=_particles_buffer
                )
                _ptr_c_particles_buffer = (
                    _internal_particles_buffer.pointer
                )
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        _until_turn_elem_by_elem = st_buffer_size_t(
        ...
        if needs_output_buffer:
            ...
            if ret != 0:
                raise RuntimeError(
                    "Error pre-calculating out buffer params"
                )
            ...
            if (
                ...
                if (
                    _output_buffer is None
                    and _internal_output_buffer is None
                ):
                    ...
                    raise ValueError(
                        "No valid output buffer available"
                    )

                if _internal_output_buffer is None:
                    raise ValueError(
                        "Could not provide output buffer"
                    )
        ...
        if (
            ...
            (
                _output_buffer is not None
                or _internal_output_buffer is not None
            )
        ...
        if self._last_status == st_ARCH_STATUS_SUCCESS.value:
            ...
            self._internal_particles_buffer = (
                _internal_particles_buffer
            )
    ...
    def is_collecting_particles(self):
        return st_TrackJobNew_is_collecting_particles(
            self._ptr_track_job
        )

    @property
    def is_collecting_beam_elements(self):
        return st_TrackJobNew_is_collecting_beam_elements(
            self._ptr_track_job
        )

    @property
    def is_collecting_output(self):
        return st_TrackJobNew_is_collecting_output(
            self._ptr_track_job
        )
    ...
    def has_particle_addresses(self):
        return st_TrackJobNew_has_particle_addresses(
            self._ptr_track_job
        )

    @property
    def num_particle_sets(self):
        return st_TrackJobNew_get_num_particle_sets(
            self._ptr_track_job
        )

    @property
    def total_num_particles(self):
        return st_TrackJobNew_get_total_num_of_particles(
            self._ptr_track_job
        )
    ...
    def has_elem_by_elem_output(self):
        return st_TrackJobNew_has_elem_by_elem_output(
            self._ptr_track_job
        )
    ...
    def num_beam_monitors(self):
        return st_TrackJobNew_get_num_beam_monitors(
            self._ptr_track_job
        )

    @property
    def has_beam_monitor_output(self):
        return st_TrackJobNew_has_beam_monitor_output(
            self._ptr_track_job
        )
    ...
    def has_elem_by_elem_config(self):
        return st_TrackJobNew_has_elem_by_elem_config(
            self._ptr_track_job
        )
    ...
    def collect(self, collect_flags=None):
        ...
        if collect_flags > 0:
            collected_flags = st_TrackJobNew_collect(
                self._ptr_track_job
            )
        return self
    ...
    def collect_particles(self):
        ...
        assert (
            self._internal_particles_buffer.pointer != st_NullBuffer
        )
    ...
    def push_particles(self):
        self._last_status = st_TrackJobNew_push_particles(
            self._ptr_track_job
        )
        raise_error_if_status_not_success(
            self._last_status,
...
class TrackJob(object):
    ...
    @staticmethod
    def num_available_nodes(
        arch_str=None, env_var_name=None, filter_str=None
    ):
        if (
            not (arch_str is None)
            and arch_str == "opencl"
    ...
    def enabled_archs():
        enabled_archs = [
            arch_str
            for arch_str, flag in SIXTRACKLIB_MODULES.items()
            if flag
        ]
    ...
    @staticmethod
    def print_nodes(
        arch_str, all=False, filter_str=None, env_var_name=None
    ):
        ...
        if stconf.SIXTRACKLIB_MODULES.get(arch_str, False):
            if arch_str == "opencl":
                if not all:
                    capacity = st.st_OpenCL_get_available_nodes_required_str_capacity(
                        _filter_str, _env_var_name
                    )
                else:
                    capacity = (
                ...
                if (
                    _status == st.st_ARCH_STATUS_SUCCESS.value
                    and capacity > 0
                ):
                ...
                print(
                    "nodes not available for architecture {0}".format(
                        arch_str
                    )
                )
        else:
            print(
                "architecture {0} is not enabled/known".format(
                    arch_str
                )
            )
    ...
    def __init__(
        ...
        if beam_elements_buffer is not None:
            beam_elements_buffer = get_cbuffer_from_obj(
                beam_elements_buffer
            )
            self._beam_elements_buffer = beam_elements_buffer
            ...
            if self._ptr_c_beam_elements_buffer == st.st_NullBuffer:
                raise ValueError(
                    "Issues with input beam elements buffer"
                )
        ...
        if needs_output_buffer:
            ...
            if ret == 0:
                ...
                if self._ptr_c_output_buffer == st.st_NullBuffer:
                    raise ValueError(
                        "Unable to map (optional) output buffer"
                    )
            else:
                raise ValueError(
                    "Error pre-calculating out buffer params"
        ...
        self.ptr_st_track_job = st.st_TrackJob_new_with_output(
            ...
            until_turn_elem_by_elem,
            ct.c_char_p(config_str),
        )

        if self.ptr_st_track_job == st.st_NullTrackJob:
            raise ValueError(
                "unable to construct TrackJob from arguments"
            )
    ...
    @property
    def requires_collecting(self):
        return st.st_TrackJob_requires_collecting(
            self.ptr_st_track_job
        )
    ...
    @property
    def can_fetch_particle_addresses(self):
        return st.st_TrackJob_can_fetch_particles_addr(
            self.ptr_st_track_job
        )

    @property
    def has_particle_addresses(self):
        return st.st_TrackJob_has_particles_addr(
            self.ptr_st_track_job
        )
    ...
    @property
    def num_beam_monitors(self):
        return st.st_TrackJob_get_num_beam_monitors(
            self.ptr_st_track_job
        )

    @property
    def has_elem_by_elem_output(self):
        return st.st_TrackJob_has_elem_by_elem_output(
            self.ptr_st_track_job
        )

    @property
    def has_beam_monitor_output(self):
        return st.st_TrackJob_has_beam_monitor_output(
            self.ptr_st_track_job
        )
    ...
    def has_stored_buffers(self):
        return st.st_TrackJob_has_stored_buffers(
            self.ptr_st_track_job
        )

    @property
    def num_stored_buffers(self):
        return st.st_TrackJob_num_stored_buffers(
            self.ptr_st_track_job
        )

    @property
    def min_stored_buffer_id(self):
        return st.st_TrackJob_min_stored_buffer_id(
            self.ptr_st_track_job
        )

    @property
    def max_stored_buffer_id(self):
        return st.st_TrackJob_max_stored_buffer_id(
            self.ptr_st_track_job
        )
    ...
    def add_program(self, path_to_program_file, compile_options):
        if not self.allows_add_program:
            raise RuntimeError(
                "Can not add a program to this TrackJob"
            )
        success = False
        program_id = st.st_ARCH_ILLEGAL_PROGRAM_ID.value

        path_to_program_file = path_to_program_file.strip().encode(
            "utf-8"
        )
        ...
        if self.arch_str == "opencl" and SIXTRACKLIB_MODULES.get(
            ...
            _controller = st.st_TrackJobCl_get_context(
                self.ptr_st_track_job
            )
            program_id = st.st_ClContextBase_add_program_file(
                _controller,
                ct.c_char_p(path_to_program_file),
    ...
    def enable_kernel(self, program_id, kernel_name):
        if not self.allows_enable_kernel:
            raise RuntimeError(
                "Can not enable a kernel at this TrackJob"
            )
    ...
    def total_num_assign_items(self):
        return st.st_TrackJob_total_num_assign_items(
            self.ptr_st_track_job
        )
    ...
    def ptr_assign_address_item(
        ...
        elif not (dest_buffer_id is None) and not (
            src_buffer_id is None
        ):
    ...
    def commit_address_assignments(self):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "Unable to commit address assignment items"
            )
    ...
    def assign_addresses(self, dest_buffer_id, src_buffer_id):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                "Unable to perform assignment of address items"
            )
        return self
    ...
    def add_stored_buffer(
        ...
        if not (cbuffer is None):
            ...
            not (ptr_c_buffer is None)
            and ptr_c_buffer != st_NullBuffer
        ):
        ...
        if (
            not (_st_buffer is None)
            and _st_buffer.pointer != st_NullBuffer
        ):
    ...
    def push_stored_buffer(self, buffer_id):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                f"Unable to push stored buffer {buffer_id}"
            )
        return self
    ...
    def collect_stored_buffer(self, buffer_id):
        ...
        if self._last_status != st_ARCH_STATUS_SUCCESS.value:
            raise RuntimeError(
                f"Unable to collect stored buffer {buffer_id}"
            )
        return self
