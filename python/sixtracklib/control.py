...
class NodeId(object):
    ...
    def to_string(node_id):
        ...
        _status = _st_NodeId_to_string(
            _ptr_node_id, _int_str, _max_str_len
        )
        node_id_str = None
    ...
    def is_valid(self):
        return (
            self._ptr_node_id != st_NullNodeId
            and st_NodeId_is_valid(self._ptr_node_id)
        )
    ...
    def platform_id(self):
        if self._ptr_node_id == st_NullNodeId:
            raise ValueError(
                "Attempting to read from uninitialized NodeId"
            )
...
class NodeInfoBase(object):
    def __init__(
        self, ptr_node_info=st_NullNodeInfoBase, owns_ptr=True
    ):
...
class ArgumentBase(object):
    def __init__(
        self, ptr_argument=st_NullArgumentBase, owns_ptr=True
    ):
    ...
    def ptr_buffer(self):
        ...
        if st_Argument_uses_cobjects_buffer(self._ptr_argument):
            ptr_buffer = st_Argument_get_cobjects_buffer(
                self._ptr_argument
            )
        return ptr_buffer
    ...
    def buffer_slot_size(self):
        return st_Argument_get_cobjects_buffer_slot_size(
            self._ptr_argument
        )
    ...
    def requires_argument_buffer(self):
        return st_Argument_requires_argument_buffer(
            self._ptr_argument
        )
    ...
    def send(
        ...
        if ptr_buffer != st_NullBuffer:
            ...
            self._last_status = st_Argument_send_again(
                self._ptr_argument
            )
    ...
    def receive_raw_argument(
        self, ptr_raw_arg_begin, raw_arg_capacity
    ):
        return self.receive(
            ptr_raw_arg_begin=ptr_raw_arg_begin,
...
class ControllerBase(object):
    def __init__(
        self, ptr_controller=st_NullControllerBase, owns_ptr=True
    ):
    ...
    def enable_debug_mode(self):
        self._last_status = st_Controller_enable_debug_mode(
            self._ptr_ctrl
        )
        return self

    def disable_debug_mode(self):
        self._last_status = st_Controller_disable_debug_mode(
            self._ptr_ctrl
        )
        return self
    ...
    def kernel_name(self, kernel_id):
        ...
        if _kernel_name_cstr != st_NullChar:
            _kernel_name = bytes(_kernel_name_cstr.value).decode(
                "utf-8"
            )
        return _kernel_name
    ...
    def send(
        ...
        if ptr_buffer != st_NullBuffer:
            ...
            ptr_raw_arg_begin != st_Null and raw_arg_size is not None
        ):
    ...
    def send_detailed(
        self, ptr_argument, ptr_raw_arg_begin, raw_arg_size
    ):
        return self.send(
    ...
    def receive(
        ...
        if ptr_argument != st_NullArgumentBase:
            ...
            if ptr_buffer != st_NullBuffer:
                ...
                ptr_raw_arg_begin != st_Null
                and raw_arg_capacity is not None
            ):
                self._last_status = st_Controller_send_detailed(
                    self._ptr_ctrl,
                    ptr_raw_arg_begin,
    ...
    def receive_buffer(self, ptr_buffer, ptr_argument):
        return self.receive(
            ptr_argument=ptr_argument, ptr_buffer=ptr_buffer
        )
    ...
    def buffer_arg_needs_remapping(self, argument):
        ...
        ptr_argument = st_NullArgumentBase
        if argument is not None and isinstance(
            argument, ArgumentBase
        ):
...
class NodeControllerBase(ControllerBase):
    def __init__(
        ...
        super().__init__(
            ptr_controller=ptr_controller, owns_ptr=owns_ptr
        )
    ...
    def available_node_indices(ctrl=None):
        ...
        if _ptr_ctrl != st_NullControllerBase:
            av_num_nodes = NodeControllerBase.num_available_nodes(
                _ptr_ctrl
            )

            if av_num_nodes > 0:
                _temp_node_indices_t = st_node_index_t * int(
                    av_num_nodes
                )
                _temp_node_indices = _temp_node_indices_t()
                _status = st_Controller_get_available_node_indices(
                    _ptr_ctrl,
                    st_arch_size_t(av_num_nodes),
                    _temp_node_indices,
                )
                if _status == st_ARCH_STATUS_SUCCESS.value:
    ...
    def available_node_ids(ctrl=None):
        _node_ids = []
        available_node_indices = NodeControllerBase.available_node_indices(
            ctrl
        )

        if available_node_indices and len(available_node_indices) > 0:
            _ptr_ctrl = st_NullControllerBase
            if ctrl is not None and isinstance(
                ctrl, NodeControllerBase
            ):
                _ptr_ctrl = ctrl.pointer
            elif ctrl is not None:
                _ptr_ctrl = ctrl
            ...
            if _ptr_ctrl != st_NullControllerBase:
                for idx in available_node_indices:
                    ...
                    if _ptr_node != st_NullNodeId:
                        _node_ids.append(
                            NodeId(ext_ptr_node_id=_ptr_node)
                        )
        ...
        if available_node_indices and len(available_node_indices) > 0:
            ...
            if _ptr_ctrl != st_NullControllerBase:
                for idx in available_node_indices:
                    ...
                    if _ptr_node != st_NullNodeId:
                        _node_id_strs.append(
                            NodeId.to_string(_ptr_node)
                        )
    ...
    def min_node_index(self):
        return st_Controller_get_min_available_node_index(
            self._ptr_ctrl
        )

    @property
    def max_node_index(self):
        return st_Controller_get_max_available_node_index(
            self._ptr_ctrl
        )
    ...
    def default_node_id(self):
        return NodeId(
            ext_ptr_node_id=st_Controller_get_default_node_id(
                self._ptr_ctrl
            )
        )
    ...
    def can_change_node_directly(self):
        return st_Controller_can_directly_change_selected_node(
            self._ptr_cltr
        )
    ...
    def are_default_node_by_platform_id_and_device_id(
        self, pid, dev_id
    ):
        return self.is_default_node_check(
            platform_id=pid, device_id=dev_id
        )
    ...
    def get_node_info_by_platform_id_and_device_id(self, pid, dev_id):
        return self.get_node_info_disp(
            platform_id=pid, device_id=dev_id
        )
    ...
    def is_selected_node_check(
        ...
        if (
            ...
            if (
                ...
                is_selected_node = bool(
                    node_index == _selected_node_index
                )
            elif node_id is not None and isinstance(node_id, NodeId):
    ...
    def are_selected_node_platform_id_and_device_id(
        self, pid, dev_id
    ):
        return self.is_selected_node_check(
            platform_id=pid, device_id=dev_id
        )
    ...
    def are_platform_id_and_device_id_available(self, pid, dev_id):
        return self.is_node_available_check(
            platform_id=pid, device_id=dev_id
        )
    ...
    def change_selected_node(
        self, current_node_index, new_selected_index
    ):
    ...
    def unselect_node_by_platform_id_and_device_id(self, pid, dev_id):
        return self.unselect_node_disp(
            platform_id=pid, device_id=dev_id
        )
