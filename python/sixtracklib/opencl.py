...
if SIXTRACKLIB_MODULES.get("opencl", False):
    ...
    class ClNodeId(object):
        ...
        def PTR_TO_STRING(
            ...
            if status == st_ARCH_STATUS_SUCCESS.value:
                ...
                if b"\0" in _node_id_str_bytes:
                    _node_id_str_bytes = _node_id_str_bytes.split(
                        b"\0", 1
                    )[0]
                node_id_str = _node_id_str_bytes.strip().decode(
                    "utf-8"
                )
            else:
                raise RuntimeError(
        ...
        def __init__(
            ...
            if ext_ptr_node_id != st_NullClNodeId:
                ...
                if not (node_id_str is None):
                    _node_id_str_bytes = node_id_str.strip().encode(
                        "utf-8"
                    )
                ...
                elif not (platform_id is None) and not (
                    device_id is None
                ):
                    st_ComputeNodeId_set_platform_id(
                        self._ptr_node_id,
        ...
        def __del__(self):
            if (
                self._owns_node
                and self._ptr_node_id != st_NullClNodeId
            ):
        ...
        def to_string(
            self, format=st_NODE_ID_STR_FORMAT_ARCHSTR.value
        ):
    ...
    class ClController(object):
        ...
        def NUM_AVAILABLE_NODES(filter_str=None, env_var_name=None):
            ...
            if not (env_var_name is None):
                _env_var_name_bytes = env_var_name.strip().encode(
                    "utf-8"
                )
        ...
        def GET_AVAILABLE_NODES(
            ...
            if _num_avail_nodes > 0:
                if not (filter_str is None):
                    _filter_str_bytes = filter_str.strip().encode(
                        "utf-8"
                    )
                    _filter_str = ct.c_char_p(_filter_str_bytes)
                else:
        ...
        def GET_AVAILABLE_NODE_ID_STRS(
            ...
            if _num_avail_nodes > skip_first_num_nodes:
                ...
                if _num_retrieved_nodes > 0:
                    ...
                    _node_id_cstr = ct.create_string_buffer(
                        _node_id_cstr_cap
                    )
        ...
        def selected_node_platform_id(self):
            platform_id = None
            _info = st_ClContextBase_get_selected_node_info(
                self._ptr_ctrl
            )
            if _info != st_NullClNodeInfo:
                platform_id = st_ComputeNodeInfo_get_platform_id(
                    _info
                )
            return platform_id
        ...
        def selected_node_platform(self):
            ...
            if _info != st_NullClNodeInfo:
                _platform_c_str = st_ComputeNodeInfo_get_platform(
                    _info
                )
                if _platform_c_str != st_NullChar:
                    node_platform_str = bytes(
                        _platform_c_str.value
                    ).decode("utf-8")
        ...
        def selected_node_name(self):
            ...
            if _info != st_NullClNodeInfo:
                ...
                if _name_c_str != st_NullChar:
                    node_name = bytes(_name_c_str.value).decode(
                        "utf-8"
                    )
            return node_name
        ...
        def selected_node_description(self):
            ...
            if _info != st_NullClNodeInfo:
                ...
                if _desc_c_str != st_NullChar:
                    description = bytes(_desc_c_str.value).decode(
                        "utf-8"
                    )
            return description
        ...
        def add_program_file(
            ...
            if self._ptr_ctrl != st_NullClContextBase:
                _path_to_prog_bytes = path_to_program.strip().encode(
                    "utf-8"
                )
                _compile_defs_bytes = compile_defs.strip().encode(
                    "utf-8"
                )
        ...
        def find_kernel_by_name(self, kernel_name):
            ...
            if self._ptr_ctrl is not st_NullClContextBase:
                _kernel_name_bytes = kernel_name.strip().encode(
                    "utf-8"
                )
        ...
        def program_path_to_file(self, program_id):
            ...
            if (
                ...
                if _path_to_prog != st_NullChar:
                    path_to_program = bytes(_path_to_prog).decode(
                        "utf-8"
                    )
        ...
        def set_kernel_arg_value(
            self, kernel_id, arg_index, val_p, val_size
        ):
            if (
                kernel_id != st_ARCH_ILLEGAL_KERNEL_ID.value
        ...
        def argument_of_kernel(self, kernel_id, arg_index):
            ...
            if _ptr_arg != st_NullClArgument:
                return ClArgument(
                    ext_ptr_arg=_ptr_arg, owns_ptr=False
                )
            else:
                error_msg = f"""
                ...
                raise ValueError(error_msg)
                self._last_status = (
                    st_ARCH_STATUS_GENERAL_FAILURE.value
                )
        ...
        def run_kernel(
            self, kernel_id, num_work_items, work_group_size=None
        ):
            _kernel_id = st_arch_kernel_id_t(kernel_id)
            ...
            if ret:
                ...
                error_msg = (
                    f"Unable to run OpenCL kernel id={kernel_id}"
                )
                raise RuntimeError(error_msg)
            return self
        ...
        def has_feature_flag(self, feature_flag_key):
            _feature_flag_key_bytes = feature_flag_key.strip.encode(
                "utf-8"
            )
            return (
                self._ptr_ctrl is not st_NullClContextBase
        ...
        def feature_flag_repr(
            self, feature_flag_key, prefix="-D", sep="="
        ):
            feature_repr = None
            _feature_flag_key_bytes = feature_flag_key.strip.encode(
                "utf-8"
            ...
            if self._ptr_ctrl is not st_NullClContextBase:
                ...
                if _requ_capacity > 0:
                    _temp_buffer = ct.create_string_buffer(
                        _requ_capacity
                    )
        ...
        def set_feature_flag(
            self, feature_flag_key, feature_flag_value
        ):
            _feature_flag_key_bytes = feature_flag_key.strip.encode(
                "utf-8"
            )
            _feature_flag_val_bytes = feature_flag_value.strip.encode(
                "utf-8"
            )
            if self._ptr_ctrl is not st_NullClContextBase:
                st_ClContextBase_set_feature_flag(
        ...
        def set_default_compile_options(self, new_compile_options):
            _compile_options_bytes = new_compile_options.strip().ecode(
                "utf-8"
            )
            if self._ptr_ctrl is not st_NullClContextBase:
                st_ClContextBase_set_default_compile_options(
                    self._ptr_ctrl,
                    ct.c_char_p(_compile_options_bytes),
                )
    ...
    class ClArgument(object):
        ...
        @property
        def controller(self):
            _ptr_ctrl = st_ClArgument_get_ptr_to_context(
                self._ptr_arg
            )
            return ClController(
                ext_ptr_ctrl=_ptr_ctrl, owns_ptr=False
            )
        ...
        def ptr_buffer(self):
            ...
            if st_ClArgument_uses_cobj_buffer(self._ptr_arg):
                _ptr_buffer = st_ClArgument_get_ptr_cobj_buffer(
                    self._ptr_arg
                )
            if _ptr_buffer is st_NullBuffer:
                raise RuntimeError(
                    "Unable to retrieve buffer from argument"
                )
    ...
    def receive(
        ...
        raise_error_if_status_not_success(
            status,
            "unsuccessful receive op; status:{0}".format(status),
        )
        self._last_status = status
    ...
    class ClController(object):
        ...
        def NUM_ALL_NODES():
            raise RuntimeError(
                "OpenCL module disabled, no nodes present"
            )
            return 0
        ...
        def PRINT_ALL_NODES():
            raise RuntimeError(
                "OpenCL module disabled, no nodes to print"
            )

        @staticmethod
        def NUM_AVAILABLE_NODES(filter_str=None, env_var_name=None):
            raise RuntimeError(
                "OpenCL module disabled, no nodes available"
            )
            return 0
