...
TEST( C99_CommonBufferMem, InitAndReserve )
{
    ...
    ptr_to_addr_t slot_it = reinterpret_cast< ptr_to_addr_t >(
        st_BufferMem_get_ptr_to_section_data( begin, SLOTS_ID, slot_size ) );
    ...
    st_BufferMem_set_section_num_entities(
    ...
    ptr_to_obj_t obj_it  = reinterpret_cast< ptr_to_obj_t >(
        st_BufferMem_get_ptr_to_section_data( begin, OBJECTS_ID, slot_size ) );
    ...
    size_t cur_num_objects  = st_BufferMem_get_section_num_entities(
        begin, OBJECTS_ID, slot_size );

    size_t cur_num_slots    = st_BufferMem_get_section_num_entities(
        begin, SLOTS_ID, slot_size );

    size_t cur_num_dataptrs = st_BufferMem_get_section_num_entities(
        begin, DATAPTRS_ID, slot_size );

    size_t cur_num_garbage_range = st_BufferMem_get_section_num_entities(
    ...
    slot_end = reinterpret_cast< ptr_to_addr_t >(
        st_BufferMem_get_ptr_to_section_end( begin, SLOTS_ID, slot_size ) );

    ASSERT_TRUE( std::distance( slot_it, slot_end ) ==
        static_cast< std::ptrdiff_t >( st_BufferMem_get_section_num_entities(
            begin, SLOTS_ID, slot_size ) ) );
    ...
    obj_end = reinterpret_cast< ptr_to_obj_t >(
        st_BufferMem_get_ptr_to_section_end( begin, OBJECTS_ID, slot_size ) );
