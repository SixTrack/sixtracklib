...
TEST( C99_CudaWrappersTrackParticlesLineTests,
      TrackSingleParticleSetAndCompareWithCpuTracking )
{
    ...
    buf_size_t const UNTIL_TURN = buf_size_t{ 25 };
        ...
        for( node_index_t const ii : available_indices )
        {
            ...
            kernel_id_t const kernel_id =
                ::NS(CudaController_add_kernel_config_detailed)(
                    ctrl, kernel_name.c_str(), buf_size_t{ 7 },
                ...
                for( buf_size_t jj = buf_size_t{ 0 } ; jj < num_chunks ; ++jj )
                {
                    ...
                    ::NS(Track_particles_line_cuda_wrapper)(
                        ptr_kernel_config, particles_arg, track_pset_index,
                            beam_elements_arg, be_begin_idx, be_end_idx,
                            finish_turn, result_arg );
                }
            }
