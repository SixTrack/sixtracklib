...
TEST( CXX_CudaTrackJobCollectPushTests, TestCollectAndPush )
{
    ...
    for( auto const& node_id : avail_node_ids )
    {
        ...
        track_job_t job( node_id.toString(), pb, eb );

        ASSERT_TRUE( job.ptrCudaController() != nullptr );
        ASSERT_TRUE( job.hasSelectedNode() );
        ASSERT_TRUE( job.ptrCudaController()->isSelectedNode( node_id ) );

        auto node_info = job.ptrCudaController()->ptrNodeInfo( node_id );
...
/* end: tests/sixtracklib/cuda/track/test_track_job_collect_push_cxx.cpp */
