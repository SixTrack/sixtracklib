    ...
    NS(ArgumentBase)* SIXTRL_RESTRICT result_arg )
{
    ...
    NS(buffer_size_t) const cmp_num_particle_sets =
        NS(Particles_buffer_get_num_of_particle_blocks)( cmp_particles_buffer );
        
    if( ( particles_arg != SIXTRL_NULLPTR ) &&
        ( particles_buffer != SIXTRL_NULLPTR ) &&
        ( cmp_particles_buffer != SIXTRL_NULLPTR ) &&
        ( cmp_num_particle_sets > ( NS(buffer_size_t) )0u ) )
    {
        /* Ensure that we are operating on the data returned by the argument
         * and not some remnant state already present in the particle_buffer) */
        ...
        if( status == NS(ARCH_STATUS_SUCCESS) )
        {
            ...
            if( ( num_particle_sets == ( NS(buffer_size_t) )0u ) ||
                ( num_particle_sets != cmp_num_particle_sets ) ||
                ( NS(Buffer_get_num_of_objects)( particles_buffer ) <
                  cmp_num_particle_sets ) )
            {
                status = NS(ARCH_STATUS_GENERAL_FAILURE);
            }
        }
        ...
        if( status == NS(ARCH_STATUS_SUCCESS) )
        {
            NS(buffer_size_t) const nn = NS(Buffer_get_num_of_objects)(
                particles_buffer );
            
            NS(buffer_size_t) num_compared_particle_sets = 
                ( NS(buffer_size_t) )0u;
            ...
            for( ; ii < nn ; ++ii )
            {
                ...
                ++num_compared_particle_sets;
            }
            
            if( ( status == NS(ARCH_STATUS_SUCCESS) ) &&
                ( cmp_num_particle_sets != num_compared_particle_sets ) )
            {
                status = NS(ARCH_STATUS_GENERAL_FAILURE);
            }
        }
