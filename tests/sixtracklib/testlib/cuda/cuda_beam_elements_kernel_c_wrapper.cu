...
__host__ int NS(Run_test_copy_beam_elements_buffer_kernel_cuda_grid)(
    ...
    NS(buffer_size_t) const threads_per_block )
{
    ...
    buf_size_t const num_beam_elements =
        NS(Buffer_get_num_of_objects)( in_buffer );

    SIXTRL_ASSERT( !NS(Buffer_needs_remapping)( in_buffer ) );

    if( ( num_beam_elements > ZERO_SIZE ) && ( num_blocks > ZERO_SIZE ) &&
        ( threads_per_block > ZERO_SIZE ) )
    {
            ...
            ( in_buffer_size   <= out_buffer_capacity ) )
        {
            ...
            if( success == 0 )
            {
                ...
                {
                    NS(BeamElements_clear_buffer)( out_buffer );
                }
                else
                {
                    success |= -64;
            ...
            if( success == 0 )
            {
                ...
                NS(BeamElements_copy_beam_elements_buffer_kernel_cuda)<<<
                    grid_dim, block_dim >>>(
...
__host__ int NS(Run_test_copy_beam_elements_buffer_kernel_cuda)(
    SIXTRL_BUFFER_ARGPTR_DEC NS(Buffer) const* SIXTRL_RESTRICT in_buffer,
    SIXTRL_BUFFER_ARGPTR_DEC NS(Buffer)* SIXTRL_RESTRICT out_buffer )
{
    int success = -131072;
    ...
    *  for reference! */
    ...
    if( cudaSuccess == cudaOccupancyMaxPotentialBlockSize( &min_grid_size,
            &block_size, NS(BeamElements_copy_beam_elements_buffer_kernel_cuda),
                0u, 0u ) )
    {
        success = 0;
    }
    ...
    if( success == 0 )
    {
        success = NS(Run_test_copy_beam_elements_buffer_kernel_cuda_grid)(
...
/* end: tests/sixtracklib/cuda/details/cuda_beam_elements_kernel_c_wrapper.cu */
