...
if __name__ == '__main__':
    ...
    # ------------------------------------------------------------------------
    initial_particles = pb.get_object(0, cls=st.Particles)
    ...
    for ii in range(nn):
        ...
        _job_particles = output_buffer.get_object(ii, cls=st.Particles)
        assert 0 == st.particles.compareParticlesDifference(
            _cmp_particles, _job_particles, abs_treshold=ABS_DIFF)
    ...
    particles_buffer = job.particles_buffer
    track_particles = particles_buffer.get_object(0, cls=st.Particles)
