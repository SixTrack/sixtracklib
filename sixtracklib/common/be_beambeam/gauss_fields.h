    ...
    SIXTRL_ARGPTR_DEC SIXTRL_REAL_T* Ey)
{
  ...
  if (r2<1e-20) temp = sqrt(r2)/(2.*NS(PI)*NS(EPSILON_0)*sigma); //linearised
  else          temp = (1-exp(-0.5*r2/(sigma*sigma)))/(2.*NS(PI)*NS(EPSILON_0)*r2);
        ...
        SIXTRL_ARGPTR_DEC SIXTRL_REAL_T* Ey_out)
{
  ...
  if (sigmax>sigmay){
    ...
    factBE = 1./(2.*NS(EPSILON_0)*NS(SQRT_PI)*S);
...
SIXTRL_INLINE void get_Ex_Ey_Gx_Gy_gauss(
    ...
    if (fabs(sigma_x-sigma_y)< min_sigma_diff){
        ...
        if(skip_Gs){
          ...
          Gx = 1/(2.*(x*x+y*y))*(y*Ey-x*Ex+1./(2*NS(PI)*NS(EPSILON_0)*sigma*sigma)
                            *x*x*exp(-(x*x+y*y)/(2.*sigma*sigma)));
          Gy = 1./(2*(x*x+y*y))*(x*Ex-y*Ey+1./(2*NS(PI)*NS(EPSILON_0)*sigma*sigma)
        ...
        if(skip_Gs){
          ...
          Gx =-1./(2*(Sig_11-Sig_33))*(x*Ex+y*Ey+1./(2*NS(PI)*NS(EPSILON_0))*\
                      (sigma_y/sigma_x*exp(-x*x/(2*Sig_11)-y*y/(2*Sig_33))-1.));
          Gy =1./(2*(Sig_11-Sig_33))*(x*Ex+y*Ey+1./(2*NS(PI)*NS(EPSILON_0))*\
