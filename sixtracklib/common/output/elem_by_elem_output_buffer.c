    ...
    NS(buffer_size_t) const out_buffer_slot_size )
{
    ...
    if( success == 0 )
    {
        success = NS(ElemByElemConfig_calculate_output_buffer_params_detailed)(
            min_part_id, max_part_id, min_elem_id, max_elem_id, min_turn_id,
                max_turn_id, until_turn_elem_by_elem, ptr_num_objects,
    ...
    NS(buffer_size_t) const out_buffer_slot_size )
{
    ...
    if( ( until_turn_elem_by_elem >= ( index_t )0u ) &&
        ( until_turn_elem_by_elem > min_turn_id ) &&
        ( input_param_valid ) )
    {
        index_t const max_elem_by_elem_turn_id =
            until_turn_elem_by_elem - ( index_t )1u;
        ...
        if( store_num_particles > ZERO )
        {
            ...
            num_slots = NS(Particles_get_required_num_slots_on_managed_buffer)(
    ...
    SIXTRL_ARGPTR_DEC NS(buffer_size_t)* SIXTRL_RESTRICT ptr_index_offset )
{
    ...
    if( ( success == 0 ) &&
        ( until_turn_elem_by_elem > ( NS(particle_index_t) )0u ) &&
        ( until_turn_elem_by_elem > min_turn_id ) )
    {
        NS(particle_index_t) const max_elem_by_elem_turn_id =
            until_turn_elem_by_elem - ( NS(particle_index_t) )1u;
