    ...
    SIXTRL_BE_ARGPTR_DEC const NS(RFMultiPole) *const SIXTRL_RESTRICT mpole )
{
    ...
    SIXTRL_BUFFER_DATAPTR_DEC NS(rf_multipole_real_t) const* bal = NS(RFMultiPole_const_bal)( mpole );
    SIXTRL_BUFFER_DATAPTR_DEC NS(rf_multipole_real_t) const* phase = NS(RFMultiPole_const_p)( mpole );
    ...
    real_t const beta0  = NS(Particles_get_beta0_value)(        particles, particle_index );
    real_t const zeta   = NS(Particles_get_zeta_value)(         particles, particle_index );
    real_t const q      = NS(Particles_get_q0_value)(           particles, particle_index ) *
                          NS(Particles_get_charge_ratio_value)( particles, particle_index );
    real_t const rvv    = NS(Particles_get_rvv_value)(          particles, particle_index );
    ...
    real_t const p0c = NS(Particles_get_p0c_value)( particles, particle_index);
    real_t const rfmultipole_energy = - q * ( (k * p0c) * dptr );

    real_t const chi    = NS(Particles_get_chi_value)( particles, particle_index );

    real_t const px_kick = - chi * dpx;
    real_t const py_kick =   chi * dpy;
    real_t const energy_kick = cav_energy + rfmultipole_energy;

    NS(Particles_add_to_px_value)( particles, particle_index, px_kick );
    NS(Particles_add_to_py_value)( particles, particle_index, py_kick );
    NS(Particles_add_to_energy_value)( particles, particle_index, energy_kick );
