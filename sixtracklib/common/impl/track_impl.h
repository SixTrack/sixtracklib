...
#define SIXTRACKLIB_DRIFT_TRACK_IMPL( T, particles, ip, length )  \
    ...
    T const ONE    = ( T )1; \
    T const TWO    = ( T )2; \
    T const rpp    = NS(Particles_get_rpp_value)(   ( particles ), ( ip ) ); \
    T const rvv    = NS(Particles_get_rvv_value)(   ( particles ), ( ip ) ); \
    T const px     = NS(Particles_get_px_value)(    ( particles ), ( ip ) ) * rpp; \
    T const py     = NS(Particles_get_py_value)(    ( particles ), ( ip ) ) * rpp; \
    T const dsigma = ( ONE - rvv * ( ONE + ( px * px + py * py ) / TWO ) );  \
        \
    T x            = NS(Particles_get_x_value)(     ( particles ), ( ip ) ); \
    T y            = NS(Particles_get_y_value)(     ( particles ), ( ip ) ); \
    T s            = NS(Particles_get_s_value)(     ( particles ), ( ip ) ); \
    T sigma        = NS(Particles_get_sigma_value)( ( particles ), ( ip ) ); \
        \
    x             += ( length ) * px; \
    y             += ( length ) * py; \
    sigma         += ( length ) * dsigma; \
...
#define SIXTRACKLIB_DRIFT_EXACT_TRACK_IMPL( T, particles, ip, length ) \
    ...
    /* ------------------------------------------------------------------------- */ \
        \
    T const ONE   = ( T )1; \
    T const delta = NS(Particles_get_delta_value)( ( particles ), ( ip ) ); \
    T const beta0 = NS(Particles_get_beta0_value)( ( particles ), ( ip ) ); \
    T sigma       = NS(Particles_get_sigma_value)( ( particles ), ( ip ) ); \
    T const px    = NS(Particles_get_px_value)(    ( particles ), ( ip ) ); \
    T const py    = NS(Particles_get_py_value)(    ( particles ), ( ip ) ); \
     \
