...
NS(track_status_t) NS(CudaTrack_track_until_turn)(
...
{
    typedef NS(track_status_t) status_t;

    status_t status = NS(TRACK_STATUS_GENERAL_FAILURE);


    return status;
}

NS(track_status_t) NS(CudaTrack_track_elem_by_elem_until_turn)(
    const NS(CudaKernelConfig) *const SIXTRL_RESTRICT kernel_config,
    NS(CudaArgument)* SIXTRL_RESTRICT particles_arg,
    NS(CudaArgument)* SIXTRL_RESTRICT beam_elements_arg,
...
NS(track_status_t) NS(CudaTrack_track_line)(
    const NS(CudaKernelConfig) *const SIXTRL_RESTRICT kernel_config,
    NS(CudaArgument)* SIXTRL_RESTRICT particles_arg,
    NS(CudaArgument)* SIXTRL_RESTRICT beam_elements_arg,
