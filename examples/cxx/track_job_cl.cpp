...
int main( int argc, char* argv[] )
{
    ...
    if( argc < 4 )
    {
        std::cout << "Usage: " << argv[ 0 ] << " DEVICE_ID_STR"
    ...
    if( ( argc >= 4 ) &&
        ...
        ( argv[ 3 ] != nullptr ) && ( std::strlen( argv[ 3 ] ) > 0u ) )
    {
        input_pb.readFromFile( argv[ 2 ] );
        eb.readFromFile( argv[ 3 ] );
    ...
    if( ( particles != nullptr ) && ( NUM_PARTICLES > 0 ) && ( NUM_TURNS > 0 ) )
    {
        ...
        track_job_t job( argv[ 1 ], pb, eb, nullptr, NUM_TURNS_ELEM_BY_ELEM );
...
/* end: examples/cxx/track_job_opencl.cpp */
